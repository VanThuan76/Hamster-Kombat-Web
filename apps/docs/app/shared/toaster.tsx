    ...
    <ToastProvider>
      {toasts.map(function ({
        ...
        return (
          <Toast
            key={id}
            className="flex max-w-[350px] h-[36px] relative box-border bg-[rgba(39,42,47,0.9)] border border-[rgba(39,42,47,0.9)] rounded-[16px] shadow-[0_0_transparent] min-w-[calc(100vw_-_32px)] items-center select-none touch-none"
            {...props}
          >
            {variant === "success" ? (
              <svg
            ...
            {title && (
              <ToastTitle className="flex-1 text-[12px] text-white font-extralight">
                {title}
              </ToastTitle>
            )}
            {description && (
              <ToastDescription className="text-[12px] font-extralight">
                {description}
              </ToastDescription>
            )}
            {action}
