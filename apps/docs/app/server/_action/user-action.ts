...
export const userLoginAction: () => UseMutationResult<
  IBaseResponse<IUser>,
  Error,
  any
> = () => {
  ...
  return useMutation<IBaseResponse<IUser>, Error>({
    ...
    onSuccess: async (data) => {
      ...
      const dataCurrentInitUser = {
        ...
        userSkins: data.data?.userSkins
          .filter((skin) => skin.is_purchased === 1)
          .map((item) => item.skin_id), //Fixed
      };
      dispatch(setInitUser(dataCurrentInitUser));
      dispatch(
        setStateEnergy({
          amount: data.data?.energy_limit || 1000,
          isReset: false,
        }),
      ); //Fixed
    },
...
export const useRankUsers: () => UseMutationResult<
  IBaseResponse<IRankUsers[]>,
  Error,
  { user_id: number }
> = () => {
    ...
    onSuccess: async (data) => {
      ...
      dispatch(
        setRanks(
          data.data.map((item) => ({
            ...item,
            image: process.env.NEXT_PUBLIC_DOMAIN_BACKEND + "/" + item.image,
          })),
        ),
      );
...
export const useUpdateRevenue: () => UseMutationResult<
  IBaseResponse<IUpdateRevenueByUser>,
  Error,
...
> = () => {
  ...
  >({
    mutationFn: (body: { user_id: number; amount: number }) =>
      axiosInstance.post<IBaseResponse<IUpdateRevenueByUser>>(
        USER_PATHS.UPDATE_REVENUE_BY_USER,
        body,
      ),
    onSuccess: async (data) => {
      if (!data.data) return;
...
export const useUpdateSkin: () => UseMutationResult<
  IBaseResponse<IUpdateSkinByUser>,
  Error,
...
> = () => {
  ...
  >({
    mutationFn: (body: { user_id: number; skin_id: number }) =>
      axiosInstance.post<IBaseResponse<IUpdateSkinByUser>>(
        USER_PATHS.UPDATE_SKIN_BY_USER,
        body,
      ),
    onSuccess: async (data) => {
      if (!data.data) return;
...
export const useFriends: () => UseMutationResult<
  IBaseResponse<IFriendUser>,
  Error,
  { id: number }
> = () => {
  const dispatch = useAppDispatch();
  ...
  return useMutation<IBaseResponse<IFriendUser>, Error, { id: number }>({
    mutationFn: ({ id }) =>
      axiosInstance.get<IBaseResponse<IFriendUser>>(
        `${USER_PATHS.FRIENDS}/${id}`,
      ),
    onSuccess: async (data) => {
      if (!data.data) return;
